...
# is a library that determines the timezone of the app 
import datetime   
#  Helps to display the code in the terminal 
import os                          
...
list_item_price = [0] * 100        
 ...
 #  these are the discounts placed on the orders 
var_discount_1 = 200                      
var_discount_2 = 1000                     
var_discount_3 = 5000                     
...
if os.name == "nt":
# makes the program runnable on windows
    navigator_symbol = "\\" 

# the global variable means having a scope throughtout the program
def def_default():
...
# prompts the user to enter option 
        ...
        if (len(input_1) == 1): 
        #The option selected must be a one letter option
        # incase  of O display order menu
            if (input_1 == 'O'):                                          
                print("\n" * 10)                                        
                def_order_menu()                                          
                break     
        # incase of  R display report of previous orders
            elif (input_1 == 'R'):                                        
                print("\n" * 10)                                        
                def_report()                                              
                break       
       # incase of P display payment details  
            elif (input_1 == 'P'):                                        
                print("\n" * 10)                                         
                def_payment()                                             
                break   
       #  incase of E end the process
            elif (input_1 == 'E'):                                        
                print("*" * 32 + "THANK YOU" + "*" * 31 + "\n")           
                break 
       #   incase none of the letters is entered display error
            else:                                                                                 
                print("\n" * 10 + "ERROR: Invalid Input (" + str(input_1) + "). Try again!")     
        else:                                                                                     
            print("\n" * 10 + "ERROR: Invalid Input (" + str(input_1) + "). Try again!")        

# this is consists of the ordering services
def def_order_menu():                                                                               
    while True:                                             
        print("*" * 31 + "ORDER PAGE" + "*" * 31 + "\n"    
              ...
              "\t(E) EXIT\n" +
              "_" * 72)
# prompts user to enter desired option
        input_1 = str(input("Please Select Your Operation: ")).upper() 
        if len(input_1) == 1:
# Incase F is chosen display food and drinks menu
            if (input_1 == 'F'):  
                print("\n" * 10)
                def_food_drink_order()
                break
# incase O is chosen display other service menu
            elif (input_1 == 'O'):
                print("\n" * 10)
                def_other_services() 
                break
# incase M is chosen display main menu
            elif (input_1 == 'M'):
                print("\n" * 10)
                def_main() 
                break
# incase E is chosen exit app and display Thank you 
            elif (input_1 == 'E'):
                print("*" * 32 + "THANK YOU" + "*" * 31 + "\n")
                break
# If any other option is chosen an error statement is displayed    
            else:
                print("\n" * 10 + "ERROR: Invalid Input (" + str(input_1) + "). Try again!") 
        else:
            print("\n" * 10 + "ERROR: Invalid Input (" + str(input_1) + "). Try again!")
...
def def_full_file_reader():                                                                        
    ...
    while i <= (len(list_foods) - 1): 
        if 'Shs' in list_foods[i]:
            list_foods[i] = str(list_foods[i][:list_foods[i].index('Shs') - 1]) + ' ' * (20 - (list_foods[i].index('Shs') - 1)) + str(list_foods[i][list_foods[i].index('Shs'):])
        i += 1

    i = 0
    while i <= (len(list_drinks) - 1):
        if 'Shs' in list_drinks[i]:
            list_drinks[i] = str(list_drinks[i][:list_drinks[i].index('Shs') - 1]) + ' ' * (20 - (list_drinks[i].index('Shs') - 1)) + str(list_drinks[i][list_drinks[i].index('Shs'):])
        i += 1

    i = 0
    while i <= (len(list_services) - 1):
        if 'Shs' in list_services[i]:
            list_services[i] = str(list_services[i][:list_services[i].index('Shs') - 1]) + ' ' * (20 - (list_services[i].index('Shs') - 1)) + str(list_services[i][list_services[i].index('Shs'):])
        i += 1
...
def def_file_sorter(): 
    ...
    while i < len(list_foods):
        list_item_price[i] = float(list_foods[i][int(list_foods[i].index("Shs") + 3):]) 
        i += 1

    i = 0
    while i < len(list_drinks):
        list_item_price[40 + i] = float(list_drinks[i][int(list_drinks[i].index("Shs") + 3):]) 
        i += 1

    i = 0
    while i < len(list_services):
        list_item_price[80 + i] = float(list_services[i][int(list_services[i].index("Shs") + 3):]) 
        i += 1
def_file_sorter()

def def_food_drink_order():
    while True:
            print("*" * 30 + "ORDER FOODS & DRINKS" + "*" * 30)
    ...
    while True:
        ...
        if total_price > var_discount_3: 
            ...
            report_new += "\n" + " " * 17 + "-" * 35 + "\n" \
                ...
                "" + " " * 17 + "DISCOUNT AMOUNTS:   Shs " + str(round(total_price * var_discount_3_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n" \
                "" + " " * 17 + "TOTAL PRICES:       Shs " + str(round(total_price, 2)) + "\n" + " " * 17 + "*" * 35 
            print(" " * 17 + "-" * 35 + "\n"
                "" + " " * 17 + "DISCOUNT RATES:      % " + str(var_discount_3_rate * 100) + "\n"
                "" + " " * 17 + "DISCOUNT AMOUNTS:   Shs " + str(round(total_price * var_discount_3_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n"
                "" + " " * 17 + "TOTAL PRICES:       Shs " + str(round(total_price, 2)))
            ...
            report_new += "\n" + " " * 17 + "-" * 35 + "\n" \
                ...
                "" + " " * 17 + "DISCOUNT AMOUNTS:   Shs " + str(round(total_price * var_discount_2_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n" \
                "" + " " * 17 + "TOTAL PRICES:       Shs " + str(round(total_price, 2)) + "\n" + " " * 17 + "*" * 35 
            print(" " * 17 + "-" * 35 + "\n"
                "" + " " * 17 + "DISCOUNT RATES:      % " + str(var_discount_2_rate * 100) + "\n"
                "" + " " * 17 + "DISCOUNT AMOUNTS:   Shs " + str(round(total_price * var_discount_2_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n"
            ...
            report_new += "\n" + " " * 17 + "-" * 35 + "\n" \
                ...
                "" + " " * 17 + "DISCOUNT AMOUNTS:   Shs " + str(round(total_price * var_discount_1_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n" \
                "" + " " * 17 + "TOTAL PRICES:       Shs " + str(round(total_price, 2)) + "\n" + " " * 17 + "*" * 35 
            print(" " * 17 + "-" * 35 + "\n"
                "" + " " * 17 + "DISCOUNT RATES:      % " + str(var_discount_1_rate * 100) + "\n"
                "" + " " * 17 + "DISCOUNT AMOUNTS:   Shs " + str(round(total_price * var_discount_1_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n"
